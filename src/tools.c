...
/*
 *  MALLOC_CHECK / CALLOC_CHECK / REALLOC_CHECK
 *
 *  like malloc(3)/calloc(3)/realloc(3) checking the result
 */

void *
Malloc_Check(size_t size, char *src_file, int src_line)
{
  void *m = malloc(size);

  if (m == NULL)
    Fatal_Error("%s:%d malloc failed", src_file, src_line);

  return m;
}

void *
Calloc_Check(size_t nb, size_t size, char *src_file, int src_line)
{
  void *m = calloc(nb, size);

  if (m == NULL)
    Fatal_Error("%s:%d calloc failed", src_file, src_line);

  return m;
}

void *
Realloc_Check(void *ptr, size_t size, char *src_file, int src_line)
{
  void *m = realloc(ptr, size);

  if (m == NULL)
    Fatal_Error("%s:%d realloc failed", src_file, src_line);
 ...
 *  RANDOM_DOUBLE1
 *
 *  Returns a random real number in [0..1] (1 included)
 *  Having [0..1) or [0..1] does not matter in general.
 */
double
Random_Double1(void)
{
...
/*
 *  RANDOM_INTERVAL_DOUBLE
 *
 *  Returns a random number in [inf..sup]
 *
 *  NB: Using Random_Double() instead of Random_Double1() 
 *  works in most cases. 
 *  Anyway having [inf..sup] or [inf..sup) (ie. sup excluded)
 *  does not matter (the probability of sup being near zero).
 *  Another possibility consists in using nextafter(3).
 */
double
Random_Interval_Double(double inf, double sup)
{
  return (Random_Double1() * (sup - inf)) + inf;
}
